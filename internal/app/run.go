...
// RunOption configures app startup.
...
// WithHandlerWrap wraps the root handler (e.g. extensions/basepath.Wrap).
func WithHandlerWrap(wrap func(http.Handler) http.Handler) RunOption {
	return func(c *runConfig) {
		c.wrap = wrap
	}
}

// Run starts the HTTP server with optional handler wrapping.
func Run(opts ...RunOption) {
	...
	rootHandler := withCORS(handlerWithSilentMode, allowedOrigins)
	if cfg.wrap != nil {
		rootHandler = cfg.wrap(rootHandler)
	}
